...
class EmbeddingChunksNode(BaseNode):
    ...
    1. 校验输入，获取 chunks 列表
    2. 按批次拼接 embedding_content（item_name + content），调用模型批量编码
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 3.3遍历chunks生成向量, step是embedding_batch_size
        final_chunks: Any = []
        for index in range(0, total, embedding_batch_size):
            #获取当前正遍历的chunks
            batch_end = index + embedding_batch_size
...
if __name__ == '__main__':
    ...
    result_state = node.process({"chunks": chunks_data['chunks']})
